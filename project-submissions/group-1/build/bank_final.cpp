...
string get_transactions(const string &username)
{
    ...
    if (!file.is_open())
    {
        return "Error opening transaction log file!";
    }

    string line, response;
    vector<string> transactions; // Vector to hold the last 10 transactions
    ...
    while (getline(file, line))
    {
        ...
        if (user == username)
        {
            string transaction = timestamp + " | " + transaction_type + " | $" + to_string(amount);
            transactions.push_back(transaction);

            // Keep only the last 10 transactions
            if (transactions.size() > 10)
            {
                transactions.erase(transactions.begin()); // Remove the oldest transaction
            }
        }
    }
    ...
    // Build the response from the last 10 transactions
...
void create_auth_file() {
    ...
    std::ofstream auth_file("bank_auth_file.txt");   // Create or overwrite the auth file
...
bool verify_auth_file(const std::string &client_auth_key) {
    ...
    std::string bank_auth_key = read_auth_file("bank_auth_file.txt");
   ...
   create_auth_file();
    ...
    while (true){
       ...
       char buffer[256] = {0};
        ...
        // cout<<atm_auth_key;
        std::string bank_auth_key = read_auth_file("bank_auth_file.txt"); // Read the bank's auth key
        // cout<<bank_auth_key;
